...
// };
const updateOrder = async (order_id, payment_id, payment_status) => {
    const order_status = payment_status === 'Success' ? 'Confirmed' : 'Pending';
    
    const query = `
        UPDATE corporate_orders
        SET paymentid = $1, payment_status = $2, corporate_order_status = $3
        WHERE corporateorder_generated_id = $4
        RETURNING *;
    `;

    const values = [payment_id, payment_status, order_status, order_id];

    try {
        const result = await client.query(query, values);
        logger.info('Corporate order updated successfully:', { order_id, payment_id, payment_status, order_status });
        return result.rows[0]; // Return the updated order details
    } catch (error) {
        logger.error('Error updating corporate order:', { error: error.message });
        throw error;
