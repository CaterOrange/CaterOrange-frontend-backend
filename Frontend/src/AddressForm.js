...
import GoogleMapComponent from './Maps/GMaps.js';
...
const AddressForm = ({ saveAddress, existingAddress }) => {
  ...
  useEffect(() => {
    const fetchDefaultDetails = async () => {
      ...
      try {
        const response = await axios.get('http://localhost:5000/address/getDefaultAddress', {
          headers: { 'token': token },
        });
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const url = existingAddress
          ? `http://localhost:5000/address/edit/${existingAddress.address_id}`
          : 'http://localhost:5000/address/createAddres';
