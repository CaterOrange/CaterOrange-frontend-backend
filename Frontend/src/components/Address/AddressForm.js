        ...
        {addresses.map((addr) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <span className="inline-block px-2 py-1 text-xs font-medium rounded-full capitalize mb-1 bg-teal-100 text-teal-800">
                  {addr.tag}
                </span>
                <p className="text-sm font-medium line-clamp-1">{addr.line1}</p>
                <p className="text-xs text-gray-600 line-clamp-1">{addr.line2}</p>
                <p className="text-xs text-gray-500 mt-1">PIN: {addr.pincode}</p>
              </div>
              {addr.media_image_url && (
                <div className="w-12 h-12 ml-2 rounded-md overflow-hidden">
                  <img
                    src={addr.media_image_url}
                    alt="Address"
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4" onClick={handleOverlayClick}>
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[85vh] overflow-y-auto" ref={modalRef}>
        ...
        <div className="px-6 py-4">
          {/* Addresses List Section - Always Visible */}
          <div className="mb-6">
            <h3 className="text-gray-800 text-sm font-medium mb-3">Saved Addresses</h3>
            {renderAddressList()}
          </div>
