...
//                           <img src={food.category_media} className=" h-full w-full" alt="Food" />
//                         </div>
...
//    'bg-green-500 text-white   focus:ring-4 focus:ring-yellow-100 font-medium rounded-full text-sm px-3 py-1 me-2 mb-2  focus:outline-none dark:focus:ring-yellow-800'  : 
//    'bg-green-100 text-green-500   focus:ring-4 focus:ring-yellow-100 font-medium rounded-full text-sm px-3 py-1 me-2 mb-2  focus:outline-none dark:focus:ring-yellow-800' }`}
// >
...
const Body = ({ isSidenavOpen, activeTab, setActiveTab }) => {
      ...
      <div className={`content ${activeTab === 'corporate' ? '' : 'hidden'}`}>
        {!showMap ? (
          <div>
            ...
            <div className={`relative ${isSidenavOpen ? 'blur-sm' : ''} z-10`}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-6 mt-8">
                {foodData.map((food, index) => (
                  <ReactCardFlip key={index} isFlipped={flipped[index]} flipDirection="horizontal">
                    {/* Front Side */}
  ...
  <div className="mt-4 w-full sm:w-auto ml-5">
    <h1 className="text-lg sm:text-xl font-semibold text-gray-700">{food.category_name}</h1>
